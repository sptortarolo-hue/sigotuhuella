...
export default function VideoGeneratorTab() {
  ...
  const musicOptions = [
    { value: 'promo_emotional.mp3', label: 'Emocional Piano' },
    { value: 'promo_latin.mp3', label: 'Latino Uplifting' },
    { value: 'promo_calm.mp3', label: 'Calma Guitar' },
    { value: 'promo_energetic.mp3', label: 'Energético Dance' }
  ];
  ...
  async function deleteVideo(id: string, filename: string) {
    ...
    try {
      ...
      if (res.ok) {
        // Also delete file if exists via API
        await fetch(`/api/admin/videos/file/${filename}`, { method: 'DELETE' }).catch(() => {}); // ignore if not implemented
  ...
  if (!canManage) {
    return (
      <div className="bg-white rounded-[2.5rem] border border-brand-accent p-8 text-center">
        ...
        <p className="text-gray-500">Solo administradores pueden acceder a esta sección.</p>
      </div>
    );
  }
    ...
    <div className="space-y-8">
      ...
      <div className="bg-white rounded-[2.5rem] border border-brand-accent p-6 sm:p-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          ...
          {/* Duration */}
          <div>
            <label className="block text-sm font-bold text-gray-600 mb-2">Duración (segundos)</label>
            <select 
          ...
          <div className="flex items-center gap-3 pt-6">
            ...
            />
            <label htmlFor="voiceEnabled" className="font-bold text-gray-700">Incluir voz en off (TTS)</label>
          </div>
        </div>
        ...
        <p className="text-xs text-gray-400 mt-3 text-center">
          La generación puede tardar hasta 2 minutos. El video aparecerá en la lista inferior automáticamente.
        </p>
      </div>
              ...
              <div key={video.id} className="rounded-2xl border border-brand-accent overflow-hidden bg-brand-bg">
                ...
                <div className="aspect-video bg-gray-200 relative">
                  ...
                  <div className="absolute inset-0 flex items-center justify-center opacity-0 hover:opacity-100 bg-black/30 transition-opacity">
                    <span className="text-white font-bold">{video.duration}s</span>
                  </div>
                </div>
                ...
                <div className="p-4">
                  ...
                  <p className="text-xs text-gray-500 mb-4">
                    {new Date(video.created_at).toLocaleDateString('es-AR')} · {video.music_track}
                  </p>
