...
async function downloadCookies() {
  ...
  try {
    ...
    console.log(`[FB Scraper] Servidor respondi├│ ${r.status}`);
...
function extractPosts(html, groupId) {
  ...
  while ((m = re.exec(html)) !== null) {
    ...
    let content = block.replace(/<script[\s\S]*?<\/script>/gi, '').replace(/<style[\s\S]*?<\/style>/gi, '')
      ...
      .replace(/Full Story|See more|Ver m├ís|Comment|Comentar|Like|Me gusta|Share|Compartir|Send|Enviar|ago[\s\S]{0,20}$/gi, '').trim();
...
async function scrapeGroup(groupId) {
  ...
  if (/input[^>]*(name="email"|type="email")/i.test(r.body)) {
    console.log('[FB Scraper] Login detectado ÔÇö cookies inv├ílidas');
...
async function main() {
  ...
  while (true) {
    ...
    if (!isWithinSchedule()) { const d = nextScheduleDelay(); console.log(`[FB Scraper] Fuera de horario. Pr├│ximo en ${Math.round(d / 60000)}min`); await sleep(d); continue; }
    ...
    console.log(`[FB Scraper] Pr├│ximo en ${Math.round(d / 60000)}min`); await sleep(d);
  }
}
