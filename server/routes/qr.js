...
router.get('/batch/:batchId/pdf', requireAdmin, async (req, res) => {
  try {
    ...
    for (let page = 0; page < identifiers.length; page += PER_PAGE) {
      ...
      doc.fontSize(14).fillColor('#5A5A40')
        .text('Sigo Tu Huella — Identificación Digital', MARGIN_X, 15, { align: 'center' });

      for (let i = 0; i < PER_PAGE && (page + i) < identifiers.length; i++) {
        const col = i % COLS;
        const row = Math.floor(i / COLS);
        const x = MARGIN_X + col * CELL_W + (col * 5);
        const y = MARGIN_Y + row * CELL_H + (row * 5);
        ...
        doc.image(qrBuffer, x + (CELL_W - QR_SIZE) / 2, y, { width: QR_SIZE });
        doc.fontSize(11).fillColor('#5A5A40')
          .text(ident.code, x, y + QR_SIZE + 4, { width: CELL_W, align: 'center' });
        doc.fontSize(7).fillColor('#999999')
          .text('sigotuhuella.online', x, y + QR_SIZE + 18, { width: CELL_W, align: 'center' });
      }
    }
